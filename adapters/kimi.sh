#!/bin/bash
# cromaiping adapter for Kimi Code CLI (MoonshotAI)
# Watches ~/.kimi/sessions/ for Wire Mode events (wire.jsonl)
# and translates them into cromaiping.sh CESP events.
#
...
# ~/Library/LaunchAgents/com.cromaizing.kimi-adapter.plist so the watcher
...
PIDFILE="$CROMAIPING_DIR/.kimi-adapter.pid"
LOGFILE="$CROMAIPING_DIR/.kimi-adapter.log"

LAUNCHD_LABEL="com.cromaizing.kimi-adapter"
...
if [ "$DAEMON_ACTION" = "uninstall" ]; then
  ...
  if [ "${KIMI_NO_LAUNCHD:-0}" != "1" ] && [ -f "$LAUNCHD_PLIST" ]; then
    ...
    rm -f "$LAUNCHD_PLIST"
    echo "cromaiping Kimi adapter LaunchAgent removed"
  fi
  ...
  if [ -f "$PIDFILE" ]; then
    ...
    if [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null; then
      kill "$pid" 2>/dev/null
      rm -f "$PIDFILE"
      echo "cromaiping Kimi adapter stopped (PID $pid)"
    else
      rm -f "$PIDFILE"
      echo "cromaiping Kimi adapter was not running (stale PID file removed)"
    fi
  else
    echo "cromaiping Kimi adapter is not running (no PID file)"
  fi
  exit 0
fi
...
if [ "$DAEMON_ACTION" = "status" ]; then
  ...
  if [ "${KIMI_NO_LAUNCHD:-0}" != "1" ] && [ -f "$LAUNCHD_PLIST" ]; then
    if launchctl list "$LAUNCHD_LABEL" &>/dev/null; then
      ...
      if [ -n "$launchd_pid" ] && [ "$launchd_pid" != "-" ]; then
        echo "cromaiping Kimi adapter is running via LaunchAgent (PID $launchd_pid)"
      else
        echo "cromaiping Kimi adapter is loaded via LaunchAgent (starting...)"
      fi
      exit 0
    else
      echo "cromaiping Kimi adapter LaunchAgent is installed but not loaded"
      exit 1
    fi
  fi
  ...
  if [ -f "$PIDFILE" ]; then
    pid=$(cat "$PIDFILE" 2>/dev/null)
    if [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null; then
      echo "cromaiping Kimi adapter is running (PID $pid)"
      exit 0
    else
      rm -f "$PIDFILE"
      echo "cromaiping Kimi adapter is not running (stale PID file removed)"
      exit 1
    fi
  else
    echo "cromaiping Kimi adapter is not running"
    exit 1
  fi
fi
...
if [ ! -f "$CROMAIPING_DIR/cromaiping.sh" ]; then
  error "cromaiping.sh not found at $CROMAIPING_DIR/cromaiping.sh"
  error "Install cromaiping first: curl -fsSL cromaizing.com/cromaiping/install | bash"
  exit 1
fi
...
if [ "$DAEMON_ACTION" = "install" ]; then
  ...
  if [[ "$(uname -s)" == "Darwin" ]] && [ "${KIMI_NO_LAUNCHD:-0}" != "1" ]; then
    # Already loaded? Done.
    if launchctl list "$LAUNCHD_LABEL" &>/dev/null; then
      echo "cromaiping Kimi adapter already running via LaunchAgent"
      exit 0
    fi
    ...
    # Capture CROMAIPING_DIR so a Kimi-direct install (under ~/.kimi/...) survives
    # reboot — without this, launchd would re-resolve to ~/.claude/... and
    # fail to find cromaiping.sh.
    CURRENT_PATH="$PATH"
    CURRENT_CROMAIPING_DIR="$CROMAIPING_DIR"
...
<dict>
    ...
    <dict>
        ...
        <key>CLAUDE_CROMAIPING_DIR</key>
        <string>${CURRENT_CROMAIPING_DIR}</string>
    </dict>
    <key>RunAtLoad</key>
    <true/>
    <key>KeepAlive</key>
...
PLIST
    ...
    echo "cromaiping Kimi adapter installed as LaunchAgent"
  ...
  if [ -f "$PIDFILE" ]; then
    old_pid=$(cat "$PIDFILE" 2>/dev/null)
    if [ -n "$old_pid" ] && kill -0 "$old_pid" 2>/dev/null; then
      echo "cromaiping Kimi adapter already running (PID $old_pid)"
      exit 0
    fi
    rm -f "$PIDFILE"
  fi
  ...
  echo "$!" > "$PIDFILE"
  echo "cromaiping Kimi adapter started (PID $!)"
  echo "  Watching: $SESSIONS_DIR"
  echo "  Log: $LOGFILE"
  echo "  Stop: bash $0 --uninstall"
  exit 0
...
try:
    ...
    # Map wire events to cromaiping.sh event names
    remap = {
        'TurnEnd': 'Stop',
...
" | bash "$CROMAIPING_DIR/cromaiping.sh" 2>/dev/null || true
}
...
# --- Start watching ---
info "${BOLD}cromaiping Kimi Code adapter${RESET}"
